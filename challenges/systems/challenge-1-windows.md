...
* Level 4+: Process execution lifecycle (how will you start and stop processes, add
  them to jobs, etc.)
...
party dependencies for key components of the challenge(job objects, output
...
1. A reusable library implementing the functionality for working with processes.
...
* CLI should be able to connect to worker service and start, stop, get status, and output of a process.
...
* Avoid concurrency and networking errors. Most of the issues we've seen in
  production are related to data races, networking error handling or resource
...
This task should be implemented in Rust and should work on 64-bit Windows machines.
